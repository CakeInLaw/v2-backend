...
__all__ = ["ModelValidator", "ModelValidatorMeta", "M_VAL"]
...
class ModelValidatorMeta(type):
    def __new__(mcs, name: str, bases: tuple[type, ...], attrs: dict[str, Any]):
        ...
        new_cls._properties = properties = {}
        for prop_schema in schema.get_properties():
            properties[prop_schema.name] = get_property_validator(schema=prop_schema, model_validator=new_cls)
        ...
        for prop_validator in properties.values():
            prop_validator.modify_model_validator()
...
class ModelValidator(Generic[M_SCH], metaclass=ModelValidatorMeta):
    ...
    @classmethod
    def get_property(cls, name: str) -> PROP_VAL:
        return cls._composites[name]
